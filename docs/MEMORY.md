...
**当前状态：MemoryCore + MemoryProxy 已从源码部署并以 launchd 常驻**（无需 Docker）：
...
- 配置：`~/.harness-memory/tdai-gateway.yaml`（standalone / SQLite / BM25 中文 / DeepSeek LLM）
  `~/.harness-memory/proxy-config.yaml`（上游 api.deepseek.com，密钥 600 权限）
- 数据：`~/.harness-memory/memory-data/`（SQLite 向量库 + FTS5 + persona 文件）
- 日志：`~/.harness-memory/logs/`
...
```bash
launchctl list | grep harness            # 查看状态
...
源码级改动记录：MemoryProxy 的 Node 版本门放宽至 v22+（原硬编码 v22.x）；better-sqlite3 升至 13.x 以兼容 Node 26。

...
- **Agent 工作台式界面（重设计）**：顶栏 = 任务标题 + 语义状态（等待回复/执行中/已完成/发生错误）；
  侧栏状态点 + 更新时间（无编号、无重复标签）；工具调用默认折叠为一行摘要，展开才见原始命令；
  生成网页后展示成果卡（打开预览 / 复制链接 / 操作说明）；底部圆角操作台（附件在容器内、模式 chip、工作目录、快捷键提示唯一）；
  暖灰浅色视觉系统（#F7F7F5 / 低饱和蓝强调 / 10–14px 圆角 / 8px 间距系统）；窄窗口侧栏自动折叠
...
```bash
npx playwright test   # 36 用例：真实 Agent 工具流（bash/写文件）/ 工作目录预览 / 工作台状态 /
                      # 工具折叠 / 成果卡 / 窄窗口 / 真实 LLM 对话 /
